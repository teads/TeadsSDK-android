...
package tv.teads.teadssdkdemo.utils.external;
...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    ...
    private TeadsLockableViewPager pager;

    /**
     * Added to prevent wrong selected position
     */
    private int mLastDisplayedItem;
    ...
    public void setViewPager(TeadsLockableViewPager pager) {
        this.pager = pager;
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            if (pager.getAdapter() instanceof CustomTabProvider) {
                ...
                tabView = LayoutInflater.from(getContext()).inflate(R.layout.pager_tab, this, false);
            }
    ...
    private Pair<Float, Float> getIndicatorCoordinates() {
        // default: line below current pager_tab
        View currentTab = tabsContainer.getChildAt(currentPosition);
        float lineLeft = currentTab.getLeft();
        float lineRight = currentTab.getRight();

        // if there is an offset, start interpolating left and right coordinates between current and next pager_tab
    ...
    /**
     * Register external OnPageChange listener
     */
    ...
    /**
     * Return the OnPageChangeListener to be used by Teads SDK
...
//            Log.d(LOG_TAG, "onPageScrolled : " + Integer.toString(position));
        ...
        public void onPageScrollStateChanged(int state) {
            int teadsPosition = pager.getDisplayedPosition();
            int currentTabPosition;

            if(pager.isAdOn()){
                currentTabPosition = mLastDisplayedItem;
            } else {
                currentTabPosition = pager.getCurrentItem();
            }

            if (state == ViewPager.SCROLL_STATE_IDLE) {
                scrollToChild(teadsPosition, 0);
            }

            // prevent wrong position of the selected tab
            if(exitState && pager.isAdOn()){
                return;
            }
            ...
            if (teadsPosition - 1 >= 0) {
                View prevTab = tabsContainer.getChildAt(currentTabPosition - 1);
                notSelected(prevTab);
            }
...
//            Log.d(LOG_TAG, "# onpageselected : " + Integer.toString(position));

            int hidedPosition = position;

            if(pager.isAdOn()){
                if(position == mAdPosition){
//                    Log.d(LOG_TAG, "-1");
                    exitState = true;
                } else if(position > mAdPosition){
//                    Log.d(LOG_TAG, "-2");
                    exitState = true;
                    hidedPosition --;
                    updateSelection(hidedPosition);
                } else {
                    exitState = false;
                    updateSelection(hidedPosition);
                }
